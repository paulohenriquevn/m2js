...
export function generateEnhancedMarkdown(
  ...
  if (options.architectureInsights || options.aiEnhanced) {
    const architectureInsights = analyzeArchitecture(allParsedFiles, dependencyGraph);
    sections.push(generateArchitectureSection(architectureInsights, parsedFile));
  }
  
  if (options.semanticAnalysis || options.aiEnhanced) {
    const semanticAnalysis = analyzeSemanticRelationships(allParsedFiles, dependencyGraph);
 ...
 */
function generateBusinessContextSection(context: any): string {
  const sections: string[] = [];
  
  sections.push('## 🎯 Business Context');
  sections.push(`**Domain**: ${context.domain} (${context.confidence}% confidence)`);
 ...
 */
function generateUsageExamplesSection(usage: any, parsedFile: ParsedFile): string {
  const sections: string[] = [];
  
  // Filter examples relevant to this file
  const relevantExamples = usage.examples.filter((example: any) => 
  ...
  // Group by category
  const categories = ['creation', 'validation', 'transformation', 'query', 'business-logic'];
  
  categories.forEach(category => {
    const categoryExamples = relevantExamples.filter((ex: any) => ex.category === category);
    
    if (categoryExamples.length > 0) {
      sections.push(`### ${category.charAt(0).toUpperCase() + category.slice(1).replace('-', ' ')}`);
      
      categoryExamples.slice(0, 2).forEach((example: any) => {
  ...
  if (usage.patterns.length > 0) {
    ...
    usage.patterns.slice(0, 3).forEach((pattern: any) => {
 ...
 */
function generateArchitectureSection(insights: any, parsedFile: ParsedFile): string {
  const sections: string[] = [];
  ...
  if (insights.layerArchitecture) {
    ...
    const currentLayer = insights.layerArchitecture.layers.find((layer: any) => 
      layer.files.includes(parsedFile.fileName)
    );
  ...
  if (insights.dataFlow.length > 0) {
    ...
    insights.dataFlow.slice(0, 2).forEach((flow: any) => {
 ...
 */
function generateSemanticSection(semantic: any, parsedFile: ParsedFile): string {
  const sections: string[] = [];
  
  // Find entities in this file
  const fileEntities = semantic.entities.filter((entity: any) => 
  ...
  fileEntities.forEach((entity: any) => {
    ...
    // State transitions
    const stateTransition = semantic.stateTransitions.find((st: any) => 
      st.entity === entity.name
    );
    ...
    // Business rules
    const entityInvariants = semantic.invariants.filter((inv: any) => 
      inv.entity === entity.name
    );
    
    if (entityInvariants.length > 0) {
      sections.push('');
      sections.push('**Business Rules**:');
      entityInvariants.forEach((inv: any) => {
  ...
  const relevantWorkflows = semantic.workflows.filter((workflow: any) => 
 ...
 */
function generateFullArchitectureSection(insights: any): string {
  const sections: string[] = [];
  ...
  insights.layerArchitecture.layers.forEach((layer: any) => {
  ...
  if (insights.designPrinciples.length > 0) {
    ...
    insights.designPrinciples.forEach((principle: any) => {
 ...
 */
function generateFullSemanticSection(semantic: any): string {
  const sections: string[] = [];
  ...
  if (semantic.entities.length > 0 && semantic.relationships.length > 0) {
    ...
    semantic.relationships.forEach((rel: any) => {
  ...
  if (semantic.workflows.length > 0) {
    sections.push('### Business Workflows');
    semantic.workflows.forEach((workflow: any) => {
...
function generateDependencyMarkdown(_graph: DependencyGraph, _options: any): string {
