...
function analyzeLayerArchitecture(
  ...
  if (controllerFiles.length > 0 && serviceFiles.length > 0 && modelFiles.length > 0) {
    pattern = 'Layered Architecture (MVC-style)';
    rationale = 'Clear separation between presentation, business logic, and data layers';
    ...
    rationale = 'Functional approach with emphasis on reusable utility functions';
  }
...
function getLayerDependencies(
  ...
  layerFiles.forEach(file => {
    const edges = dependencyGraph.edges.filter(edge => 
      edge.from === file.filePath && !edge.isExternal
    );
 ...
 */
function analyzeErrorHandling(parsedFiles: ParsedFile[]): ErrorHandlingStrategy {
  ...
  if (patterns.includes('Boolean Return Pattern') && !patterns.includes('Explicit Error Throwing')) {
...
function analyzeSecurityPatterns(parsedFiles: ParsedFile[]): SecurityPattern[] {
  ...
  const authFunctions = securityFunctions.filter(func => 
    func.name.includes('auth') || func.name.includes('login') || func.name.includes('token')
  );
...
function analyzeTestingStrategy(parsedFiles: ParsedFile[]): TestingStrategy {
  ...
  const hasComplexLogic = parsedFiles.some(file => 
    file.functions.length > 5 || file.classes.some(cls => cls.methods.length > 5)
  );
  
  return {
    approach: hasComplexLogic ? 'Unit Testing Recommended' : 'Integration Testing Focus',
 ...
 */
function analyzePerformancePatterns(parsedFiles: ParsedFile[]): PerformancePattern[] {
  ...
  const cachingFunctions = parsedFiles.flatMap(file => 
    file.functions.filter(func => 
      func.name.includes('cache') || func.name.includes('memo')
    )
  );
